...
class FinalHarness:
    ...
    def user_response(
        ...
        # The reference user_response is generated by the competition's canonical
        # per-control template (the baseline harness). Match it exactly — with the decided
        # target interpolated — so the semantic axis scores against the same wording rather
        # than a paraphrase. Composed only from the already-decided control/target fields.
