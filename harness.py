    ...
    def decide(
        ...
        if "candidates_after" in binding_order:
            if route_is_local_only(route_snapshot):
                return self._decision(ctx, "local_update", "proceed", "route_epoch_local", from_directive=True)
            if "local" in route_snapshot and "external" in route_snapshot:
                return self._decision(ctx, "clarify", "ask", "route_epoch_unresolved", from_directive=True)
