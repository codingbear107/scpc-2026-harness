...
## 정정 (독립 검토 반영, 07-12)
- LOEO 0.58은 **이 프로토타입 마이너의 한계**를 보여줄 뿐, 120개 라벨의 정보이론적 천장 증명이 아니다(full-dev gate 97/120으로 champion 117/120보다 약한 학습기의 외삽 실패). full typed-state grammar를 제대로 구현하면 결론이 달라질 수 있으나 Round-1 마감 내 불가하여 **시간 사유로** 중단을 유지한다.
- LOEO 분모(288)는 태스크 중복 포함; task-단위 재계산 0.583~0.598(독립 검토). LOSO 0.70 vs champion full-dev 0.975는 동일조건 비교가 아님.

## 이 측정이 시사한 것(하향 조정) — "천장이 왜 낮은가"에 대한 답
